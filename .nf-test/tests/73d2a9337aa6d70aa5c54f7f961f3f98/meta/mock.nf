...
include { PREPARE_INPUTS } from '/faststorage/home/michaelk/NOT_BACKUP/developer/oncoanalyser/tests/../subworkflows/local/prepare_inputs/main.nf'
...
workflow {
    ...
    input = []
                ...
                input[0] = "/faststorage/home/michaelk/NOT_BACKUP/developer/oncoanalyser/tests/fixtures/samplesheet.spring.csv"
                
    //----

    //run workflow
    PREPARE_INPUTS.run(input.toArray())
    
    if (PREPARE_INPUTS.output){

        // consumes all named output channels and stores items in a json file
        PREPARE_INPUTS.out.getNames().each { name ->
            serializeChannel(name, PREPARE_INPUTS.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  
    
        // consumes all unnamed output channels and stores items in a json file
        def array = PREPARE_INPUTS.out as List<Object>
        def i = 0
        array.each { output ->
